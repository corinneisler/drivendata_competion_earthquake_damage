...
for k in range(1, 21):
    ...
    y_pred = dtree.predict(X_test_train)
    f1 = f1_score(y_test_train, y_pred, average='micro')
    f1_list.append(f1)
print(f1_list)
# Best f1 score (0.6580): 20 features
...
# Predict y_test
y_pred = dtree.predict(X_test_best20)
...
df.to_csv("y_pred_4_decision_tree_selectKBest20_with_resampling.csv", index=False)

'''
Submission do Driven Data competition:
Micro-averaged F1 Score: 0.5787

This model has the worst performance among all the tried out models. In addition, a decision tree with 20 features is very hard to interpret.

without resampling, the ordinal regression model only predicts damage grades 2 and 3 -> not submitted to competion
